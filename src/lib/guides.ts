...
export const articles: Article[] = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: "text", text: "Awash Bank receipts are public HTML pages on awashpay.awashbank.com, served by a Java backend called MoonlightMBanking behind an F5 BIG-IP WAF. We have fully reverse-engineered the share-link structure, including the Transaction ID format embedded inside it." },
      {
        type: "table",
        headers: ["Field", "Value"],
        rows: [
          ...
          ["Response format", "HTML"],
          ["Input needed", "Full Awash share link (from app share button)"],
          ["Account number required", "No"],
          ["Geo-blocked", "No"],
      ...
      { type: "heading", text: "Transaction ID structure (new finding)" },
      { type: "text", text: "The numeric Transaction ID is not random. It has a fixed structure: YYMMDDHHMM followed by a 5-digit per-minute sequence number. For example, Transaction ID 260607112275735 breaks down as:" },
      {
        type: "table",
        headers: ["Component", "Digits", "Value", "Meaning"],
        rows: [
          ["YY", "2", "26", "Year (2026)"],
          ["MM", "2", "06", "Month (June)"],
          ["DD", "2", "07", "Day (7th)"],
      ...
      { type: "text", text: "This means the Transaction ID itself encodes the transaction date and time down to the minute. The 5-digit sequence resets each minute and is not globally monotonic. We verified this against all 5 receipt samples and the timestamp matches in every case (with a 1-minute processing delay in one sample)." },
      ...
      { type: "text", text: "We verified this with five real Awash receipts from different transaction types (Send To Bank, IPS to CBE, IPS to BOA, Merchant Payment, Send Money). In every case, converting part 1 from base-36 to decimal produced the exact Transaction ID printed on the receipt. The URL is case-insensitive (lowercase works)." },

      { type: "heading", text: "Counter analysis" },
      { type: "text", text: "Part 2 is not a hash of the Transaction ID. We tested and ruled out: CRC32, Adler32, MD5/SHA1/SHA256 truncation, Java hashCode, DJB2, FNV-1a, HMAC, affine functions, modulo operations, and bit manipulation. It is a monotonically increasing counter with a strong linear relationship to time (R-squared = 0.9998)." },
      {
        type: "table",
      ...
      { type: "text", text: "Linear regression gives a rate of ~7.15 per second (~617,000 per day) with a counter epoch around February 2025. The maximum residual is ±110K, meaning the counter can be predicted within a window of about 220,000 values for any given timestamp." },
      { type: "text", text: "The server strictly validates both segments. Replacing part 2 with a nearby value returns 'Mistmatch / invalid receipt id'. Omitting it returns 'Invalid receipt id'. Part 2 must be exactly 6 base-36 characters (5 or 7 chars returns mismatch). Extra dashes or special characters are rejected by the WAF." },

      { type: "heading", text: "Server architecture" },
      { type: "text", text: "The Awash receipt server runs a Java application called MoonlightMBanking (revealed by the JSESSIONID cookie path). It sits behind nginx and an F5 BIG-IP ASM WAF that blocks all paths except the /-{part1}-{part2} receipt URL pattern. A merchant API exists at /awashbirrpro/merchant-api/ but is also WAF-blocked. The receipt HTML contains no client-side token generation logic: the share URL is pre-baked in the HTML and only used to render a QR code." },
      {
        type: "table",
      ...
      { type: "heading", text: "Can the share link be generated from the Transaction ID?" },
      { type: "text", text: "No, not without server access. Here is why:" },
      { type: "ordered", items: [
        "Part 1 (base-36 of Transaction ID) is trivially derivable. We can compute it from any Transaction ID.",
        "Part 2 (counter) is server-generated and cannot be computed from the Transaction ID or any client-side data.",
        "The counter can be estimated from the transaction timestamp (embedded in the txID) to within ±110K values.",
      ...
      { type: "ordered", items: [
        ...
        "Does the Awash app call a specific API endpoint to generate the share link, or is it returned in the transaction response?",
        "Are there multiple URL formats? (We found a 12-char part 1 sample that does not follow the YYMMDDHHMM pattern)",
      ]},

      { type: "callout", variant: "tip", title: "Help us crack the Awash counter", text: "If you have Awash share links you can share for research (especially 2-3 from the same hour, or from different senders/branches), please open an issue on GitHub. With 20-50 samples we can narrow the counter model to ±1K and potentially predict part 2 from the Transaction ID timestamp. Even just the two-part token (e.g. 2KDL95Z0NR-4U61O6) and the transaction date/time helps." },
      { type: "callout", variant: "info", title: "Privacy note", text: "If you share Awash links, the receipts already contain the transaction details. If you want to help without exposing real data, paste just the two-part token (e.g. 2KDL95Z0NR-4U61O6) and the transaction date/time. Do not include the transaction if you are not comfortable sharing it." },

      { type: "heading", text: "How to verify Awash with cheki" },
      { type: "text", text: "Until the full URL can be generated from the Transaction ID, you must use the share link from the Awash app. Paste the full link into cheki and the parser will extract the Transaction ID, receiver, amount, date, and other fields from the official HTML receipt. cheki handles all receipt variants (Transfer successful, Transaction successful, Transaction Details) including IPS bank transfers, merchant payments, and send-money to wallet." },
